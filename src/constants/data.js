export const techSkils = [
    { name: 'HTML5',            id: 't1'},
    { name: 'CSS3',             id: 't2'},
    { name: 'SCSS',             id: 't3'},
    { name: 'GIT',              id: 't4'},
    { name: 'WebPack',          id: 't5'},
    { name: 'Gulp',             id: 't6'},
    { name: 'JavaScript',       id: 't7'},
    { name: 'jQuery',           id: 't8'},
    { name: 'PHP',              id: 't9'},
    { name: 'WordPress',        id: 't10'},
    { name: 'React.js',         id: 't11'},
    { name: 'Redux',            id: 't12'},
    { name: 'RTK Thunk',        id: 't13'},
    { name: 'RTK Query',        id: 't14'},
    { name: 'Emotion React',    id: 't15'},
    { name: 'Formik with Yup',  id: 't16'},
    { name: 'TypeScript',       id: 't17'},
    { name: 'REST',             id: 't18'},
    { name: 'SOLID',            id: 't19'},
];

export const softSkils = [
    { name: 'Scrum',            id: 's1'},
    { name: 'Agile',            id: 's2'},
    { name: 'GTD',              id: 's3'},
    { name: 'Teamwork',         id: 's4'},
];

export const profession = 'Front-end engineer';

export const name = 'Kasianchuk Maksym';

export const aboutMe = 'Hi, my name is Max, I\'m 22. I\'ve been working in web development indstry for two years at Hostpro company. In this position, I work with such technologies as CMS WordPress, php and jQuery. I have always wanted to develop my skills of web-developer, and for that I chose the React Front-end technology stack. I believe that this stack is quite progressive and convenient to work with. Now I\'m familiar with React.js with TypeScript and have started learning React Native. Yes, I have commercial web development experience, but not with React yet. While studying at the React Front-end course, I got acquainted with such technologies as React, Redux Toolkit, RTK Query and Async Thunk and TypeScript. I developed the MovieLib site for finding trending movies, which I always use when I want to find an interesting movie for the evening :)';

export const company = 'Hostpro';
export const country = 'Ukraine';
export const position = 'Frontend Web Developer';
export const period = 'From April 19, 2021 up to now';
export const duties = [
    {
        id: 'd1',
        text: 'Support of existing projects'
    },
    {
        id: 'd2',
        text: 'Development of new projects'
    },
    {
        id: 'd3',
        text: 'Creation of promotional interactive pages'
    },
];
export const description = '';

export const educationList = [
    {
        id: 'ed1',
        place: 'GoIT',
        specialty: 'Online course of Front-end React developer',
        period: 'January 2023, up to now',
    },
    {
        id: 'ed2',
        place: 'VNTU | Vinnytsia National Technical University',
        specialty: 'Master\'s degree, Security of computer and information systems/information protection',
        period: 'September 2021 - December 2022',
    },
    {
        id: 'ed3',
        place: 'VNTU | Vinnytsia National Technical University',
        specialty: 'Bachelor\'s degree, Computer Software Engineering',
        period: 'September 2017 - June 2021',
    },
];

export const comProjects = [
    {
        id: 'pcom1',
        linkUrl: 'https://hostpro.ua/ua',
        linkName: 'hostpro.ua',
        stack: 'HTML5, SCSS, WordPress, PHP, jQuery',
    }, 
    {
        id: 'pcom2',
        linkUrl: 'https://billing.hostpro.ua',
        linkName: 'billing.hostpro.ua',
        stack: 'Smarty, SCSS, PHP, jQuery',
    },
    {
        id: 'pcom3',
        linkUrl: 'https://hostpro.ua/wiki/ua',
        linkName: 'Wiki by Hostpro',
        stack: 'HTML5, SCSS, WordPress, PHP, jQuery',
    },
    {
        id: 'pcom4',
        linkUrl: 'https://hostpro.ua/blog/ua',
        linkName: 'Blog by Hostpro',
        stack: 'HTML5, SCSS, WordPress, PHP, jQuery',
    },
    {
        id: 'pcom5',
        linkUrl: 'https://uk.myhostexpert.com/',
        linkName: 'Myhostexpert',
        stack: 'HTML5, SCSS, PHP, jQuery',
    },
]; 

export const educProjects = [
    {
        id: 'ped1',
        linkUrl: 'https://mk-movielib.netlify.app/',
        linkName: 'MovieLib',
        stack: 'React.js, React-Router, Formik, Yup, Axios, Emotion',
    },
    {
        id: 'ped2',
        linkUrl: 'https://maksymkasianchuk.github.io/image-finder-on-hooks/',
        linkName: 'ImageFinder',
        stack: 'React.js, Formik, Yup, Axios, Emotion',
    },
    {
        id: 'ped3',
        linkUrl: 'https://maksymkasianchuk.github.io/phonebook-on-hooks/',
        linkName: 'Phonebook',
        stack: 'React.js, React-Router, Redux, RTK Thunk, RTK Query',
    },
];